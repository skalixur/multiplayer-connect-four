...
export default function Game() {
  ...
  useEffect(() => {
    ...
    socket.on("turn", ({ currentPlayerSymbol }) =>
      setIsYourTurn(currentPlayerSymbol === playerSymbol),
    )
    socket.on("game-over", ({ winnerSymbol, tied }) =>
      setGameOverState(tied ? "tied" : `${winnerSymbol} win`),
    )
    socket.on("player-data", ({ symbol }) => setPlayerSymbol(symbol))
    socket.on("player-join", (data) => {
      const opponent =
        data.x?.playerId !== playerId
          ? data.x
          : data.o?.playerId !== playerId
            ? data.o
            : null
      if (opponent) {
  ...
  function onRematchClick(isRematchRequested) {
    ...
    toast(
      isRematchRequested
        ...
        : "Rematch request cancelled!",
    )
  }

  const emitEvent = (event, payload) =>
    socketRef.current?.emit(event, { roomCode, playerId, ...payload })
  const handleCellClick = (rowIndex, cellIndex) =>
    isYourTurn && !gameOverState && emitEvent("make-move", { cellIndex })

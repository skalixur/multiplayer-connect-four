...
export function WinnerDisplay() {
  ...
  const winningPlayer = isPlayerWinner ? { name: playerName, symbol: playerSymbol } : { name: otherPlayerName, symbol: otherPlayerSymbol };

  const { name: winningPlayerName, symbol: winningSymbol } = winningPlayer

  const displayMessage =
    gameOverState === "tied" ? "It's a tie!" : (
      <span><PlayerName {...winningPlayer} /> wins!</span>
    );
